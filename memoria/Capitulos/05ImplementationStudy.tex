...
\chapter{Estudio de Actuadores Físicos}
...
\begin{resumen}
Los generadores kinéticos basan su funcionamiento en captar las vibraciones y movimientos del ambiente y transformarlos en energía eléctrica. Por esto de cara a las pruebas de funcionamiento hay que simular un entorno en el que se pueda controlar la frecuencia, para simular una situación de desintonizado y probar de esta manera los algoritmos de sintonizado de manera estable controlada y repetible. 
Para la realización de este entorno es necesario, una señal de frecuencia controlable y un dispositivo que transforme esa señal en vibraciones mecánicas. 
...
%------------------------------------------------------------------
A la hora de construir y probar un \ac{EHG} que produce energía eléctrica a partir de vibraciones, es necesario poder reproducir estas vibraciones. Además dado que el \ac{EHG} es muy sensible a la frecuencia de estas vibraciones dada su alta \ac{Q} se hace imprescindible poder controlar esta frecuencia de forma precisa.

Si el \ac{EHG} no vibra a su frecuencia de resonancia se dice que el generador esta desintonizado. 
Para causar esta situación es necesario poder controlar la frecuencia  la que se esta excitando el sistema. De esta manera variando la frecuencia se puede estudiar como reacciona el \ac{EHG}.

Para poder tener control sobre la frecuencia de trabajo se va a emplear un generador de señales Tektronix CFG250.
\begin{figure}[h!]
        \centering
...
%-------------------------------------------------------------------
Tras probar el generador Tektronix se han obtenido resultados positivos.

También se ha probado el generador Promax GF1000. Pero las pruebas previas no han sido tan satisfactorias como con el generador Tektronix.
\begin{figure}[h!]
        \centering
...
%------------------------------------------------------------------
Para probar el diseño hace falta una vibración controlable en frecuencia, para esto se va a recurrir a un generador de vibraciones. Este dispositivo transforma una señal sinusoidal a la entrada, en vibraciones mecánicas. Por medio de este dispositivo se puede actuar tanto sobre la amplitud de la vibración como sobre la frecuencia. El rango de frecuencias que maneja varía desde 0.1Hz a 5kHz. Consta de una bobina con un núcleo de imán permanente que se excita mediante una señal de entrada.
El generador empleado es el modelo 2185.00 de Frederiksen. Sus especificaciones técnicas son:
\begin{itemize}
...
\begin{figure}[!h]
        \centering
		\includegraphics[width=0.3\textwidth]{Imagenes/imagenes/cap5/vibgen2}  
