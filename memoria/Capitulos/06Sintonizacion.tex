...
\underline{Salida del multiplicador:} En este punto hay un nivel de tensión continua comparable a los valores de funcionamiento del microcontrolador y por lo tanto es más adecuado para tomar las medidas sin embargo el uso de condensadores provoca que la caída de la señal, cuando el generador se desintoniza, no sea inmediata, lo que provoca problemas para detectar con rapidez esta situación.

\bigskip

En la sección \ref{cap6:subsubsec:implementacion} se proponen modos de implementar ambas opciones con un micro-controlador real.
...
Este algoritmo permite tiempos en los que el micro-controlador esta dormido y por lo tanto no está consumiendo. Sin embargo si este tiempo es muy grande puede ser que, para cuando se mida la energía que se esta produciendo, el generador lleve tiempo desintonizado.
...
En situación de sintonización el micro controlador está dormido. Cuando se detecta una caída en la entrada, se dispara una interrupción que le despierta para que inicie una rutina de sintonizado. Mientras esta desintonizado el micro controlador no se duerme hasta que encuentra la posición de sintonizado.
...
Este algoritmo es el mas inmediato, es sencillo de implementar y no requiere muchos recursos del micro-controlador. La parte negativa es que en determinadas circunstancias el tiempo de sintonizado es muy grande, esto se da por ejemplo si la nueva frecuencia de resonancia se desplaza hacia el sentido opuesto al que el microcontrolador empieza a barrer. Si esto pasa, para encontrar la nueva situación de sintonización, el sintonizador primeramente tendrá que recorrer todo un lado de la lamina, para recorrer posteriormente la zona donde se halla el objetivo.
\bigskip
...
En estos, el microcontrolador conoce la frecuencia de resonancia (frecuencia de trabajo actual del generador) y es capaz de relacionarla con los cambios en la posición del sintonizador.
...
De esta manera si se detecta una desintonización el microcontrolador desplazaría el sintonizador a la siguiente frecuencia con más probabilidad, y así sucesivamente hasta volver a estar sintonizado.
...
\item El estudio de los microcontroladores disponibles y en detalle el elegido.
...
\subsubsection{Motores disponibles}
...
Las caracteristicas de estos motores se pueden consultar en el anexo [PONER EL ANEXO DONDE LAS DATASHEETS DE LOS MOTORES]
\bigskip
...
Tamaño: 7mmX6mm.
%-------------------------------------------------------------------
\subsection{Circuitos controladores de motores}
%-------------------------------------------------------------------
...
La inteligencia del sistema va a residir en el micro-controlador. Va ser el encargado de ejecutar los algoritmos propuestos.
...
\end{itemize}
A continuación se va a realizar un estudio de los microcontroladores disponibles y su idoneidad para este proyecto:

%\subsection{Estudio de micro-controladores}
\bigskip

El primer paso es dimensionar el micro, estimar la potencia de calculo, el consumo y el número de pines de entrada/salida necesarios para el proyecto. 
\bigskip

En cuanto la potencia de calculo, dado los algoritmos simples que se han propuesto, no va ser necesario que sea muy elevada y puesto que el proyecto se basa en la generación de energía, se buscará un micro de bajo consumo.
\bigskip
...
El primer microcontrolador que se estudia es el PIC 10LF320. 
\bigskip

Este micro tiene un consumo mínimo: 25$\mu$A alimentado a 1,8V.
...
Sin embargo se ha detectado que la memoria de programa es muy limitada (512 palabras), llenándose casi por completo con el algoritmo más básico lo que ha sido suficiente para causar su descarte y el paso a un micro con mas memoria.

\textbf{\underline{Conclusión del estudio:}}
\bigskip

Aunque el micro cumple con las especificaciones de potencia y consumo, su \textbf{limitada memoria de programa} hace que se busque el siguiente micro en la serie PIC 12 donde se puede encontrar uno con la suficiente memoria.

\subsubsection{PIC 12LF1571}

Los micros de la serie PIC 12 tienen mas memoria y su consumo aumenta ligeramente.
...
Llegados a este punto hay que tomar una decisión en cuanto al numero de pines de entrada/salida necesarios. Este micro dispone de 5 pines, hay que ver si son suficientes o es necesario buscar un micro-controlador con más.
\bigskip

Como se ha visto en \ref{cap6:subsubsec:drv8833}, ya solo el integrado DRV8833 necesita 5 pines para su funcionamiento lo que nos dejaría sin pines de entrada en el micro para hacer las capturas de energía producida por el generador.

Aunque en un caso de producto final la solución podría ser tan fácil como multiplexar un pin en el tiempo (para hacer medidas y para mover el motor), esto es un proyecto didáctico, y se ha decidido buscar un micro con mas pines, para que se puedan incorporar mas funcionalidades en un futuro y sea mas fácil la depuración del código de los algoritmos.

\textbf{\underline{Conclusión del estudio:}}
\bigskip

Aunque este micro es valido para el proyecto, en aras de conseguir un sistema más didáctico, se va a buscar un micro con mas pines de entrada/salida para aumentar la flexibilidad de la solución final. El siguiente paso lógico, por tanto, será ir a la serie PIC 16, con mas pines que la serie PIC 12.

\subsubsection{PIC 16LF1503}

Se ha llegado a este microcontrolador en la búsqueda de uno con mas pines que el PIC 12LF1571.
...
En el apartado \ref{cap6:subsec:propone_captura} se propusieron los métodos de monitorización de la energía producida para detectar un cambio en la frecuencia del entorno y actuar en consecuencia, pero será en esta sección donde se expondrá como se han implementado y programado en el micro-controlador.
\bigskip
...
Mediante la inclusión en el programa de la formula de la frecuencia de resonancia (\ref{cap6:equation:fresonancia}) y estableciendo una relación entre el tiempo de giro del motor y la distancia que se mueve el sintonizador, el microcontrolador puede estimar en todo momento a que frecuencia esta trabajando el sistema y en función de ella calcular el periodo de la señal que tiene que medir.


Sería además necesaria una inicialización siempre en la misma posición para fijar la frecuencia inicial para que el micro controlador supiese a partir de donde tiene que empezar a trabajar.
\bigskip
...
\item Volver a medir y ver si en un tiempo cercano a un periodo, la señal vuelve a superar el umbral
...
En este punto tiene un nivel de continua alto. Del orden de la alimentación del micro (3-5V). Por lo tanto realizar la medida aquí es bastante simple.
\bigskip

Para realizar esta medida se pueden emplear dos módulos del microcontrolador:
...
Poca capacidad de reacción a la caída de señal puesto que los condensadores que conforman el multiplicador impiden que el nivel de la señal caiga o suba bruscamente.
\end{itemize}
...
Para la ejecución de este tipo de algoritmos basta con saber la distancia que se quiere mover el sintonizador en cada iteración y aplicar la formula \ref{cap6:equ:motor} para conocer el tiempo que se tiene que activar la salida correspondiente del micro-controlador.
\bigskip
...
Implementando esta simple formula en el micro, se pueden ejecutar los algoritmos no predictivos.
...
En este caso el micro iría recorriendo las frecuencias mas usuales hasta dar con la correcta.
...
Para que se pueda emplear este método el micro-controlador tiene que conocer en todo momento la frecuencia a la que está trabajando el sistema.
\bigskip

\underline{Calibración:}

La calibración es una parte fundamental de estos algoritmos. Es la encargada de que el micro conozca en cada momento la frecuencia de trabajo. Al inicializar el algoritmo, el sintonizador se desplazaría a una posición fija para la cual el micro conoce la frecuencia que corresponde.
\bigskip

Esto se implementa mediante un sensor fin de carrera que active el sintonizador al colocarse en determinada posición. A partir de este momento el micro fija la frecuencia a determinado valor, que será el correspondiente a esa posición. Esto es posible puesto que el generador es de dimensiones fijas, por lo que el sintonizador en una posición determinada siempre asegura la misma frecuencia de resonancia en la lámina.



\section{Placa PCB desarrollada para integrar la electrónica}

Como se deduce de la sección anterior, en la PCB desarrollada se incluirá un Micro-controlador PIC 16LF1503, un driver de motores DRV8833 y un array de Darlingtons ULN2003A.
\begin{figure}[!h]
        \centering
...
Puesto que no van a estar los dos controladores conectados a la vez pueden compartir pines del microcontrolador. Esto facilita la comprensión del sistema y facilita la implementación de los algoritmos en el micro puesto que independientemente del motor y del controlador que haya conectado, este siempre usa los mismo pines.

Para que puedan compartir los pines, se incluye un mutiplexor cuadruple 2:1.

Este multiplexor tiene internamente 4 multiplexores 2:1. A cada uno de estos multiplexores internos se le conecta uno de los 4 pines del micro-controlador, y a las salidas se conectan los controladores.
\begin{figure}[!h]
        \centering
...
Llegados a este punto tenemos el esquema básico de la PCB \ref{fig:cap6:placa}.
\begin{figure}[!h]
        \centering
...
\item \textit{U4} PIC 16LF1503: Micro-controlador.
...
\subsection{Puerto para programación del PIC}

Puesto que esta placa esta concebida como una plataforma de pruebas para experimentar con distintos algoritmos, es un requisito básico que sea fácil cargar los programas desarrollados en el PIC.

EXPLICAR LO DEL RJ11 DEL ICD3 Y LA FACILIDAD DE CONEXIÓN.
