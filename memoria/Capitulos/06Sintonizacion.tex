...
\textbf{\underline{Modificación del centro de gravedad de la masa del extremo}}
...
//dibujo de masa extra desplazable.

\textbf{\underline{Variación del coeficiente de elasticidad de la lámina}}
...
//dibujo de eletroimanes acutando
...
Este metodo requiere de gran precisión en el acoplo, puesto que aunque una de las láminas tiene que ser movil, tiene que tener una fijacion perfecta con la otra para que no haya variaciones indeseadas del coeficiente de amortiguamiento. Por esta causa se considera que este método es de una dificultad técnica considerable y no será empleado.

//dibujo de laminas paralelas


\textbf{\underline{Variación de los parámetros fisicos de la lámina}}

Aunque variar el grosor y el ancho de la lámina es impracticable, modificar su longitud si es posible aunque no trivial. Por tanto la dimensión a variar será L que como se ha visto, hace que la frecuencia de resonancia (fr) cambie inversamente proporcional a L3/2.  Es decir, disminuyendo L, aumenta Fr.
//imagen de sintonizacion_mecanica (grafica)
Ahora queda estudiar la manera óptima de conseguir variar este parámetro.
 La solución inmediata requeriría soltar la lámina de la base, desplazarla y volver a ajustarla, con el consiguiente gasto de energía y pérdida de eficiencia. Este método no es práctico para un sistema que tiene que sintonizarse de forma autónoma, por esto se hace necesaria la búsqueda de soluciones que puedan ser implementadas y ejecutadas de manera autónoma a la par que eficiente en cuanto al gasto de energía necesario para llevarlas a cabo.
Otra posible solución que se pensó a la hora del diseño, pasaría por un sitema de engranages o ruedas dentadas que mediante un motor desplazasen la lamina. Esta solucion elimina el problema de soltarla y volver a ajustarla en una nueva posición, sin embargo aumenta considerablemente la dificultad de afianzarla a la base y del mecanismo en general por lo que se descartó.
//meter un dibujo de los engranajes
Estas soluciones presentadas además conllevan un problema del que no se ha hablado. Como se ha definido anteriormente, la arquitectura el generador requiere de una bobina fija en la base e imanes suspendidos de la lámina. Si se desplazase esta, los imanes se moverían respecto a la bobina perdiendo el máximo de flujo magnético necesario para generar energía.
Para contrarrestar este efecto, se podría pensar en implementar otro mecanismo que moviese la bobina a la vez que la lámina para que siempre estuviesen enfrentados imanes y bobina algo que requeriría una complejidad desmedida que además iría asociada a una pérdida de eficiencia considerable. Es por esto que se introduce una nueva idea. La idea de longitud efectiva y el sintonizador.
Longitud efectiva:
Una solución basada en esta idea no requiere el desplazamiento de la lámina, por lo tanto no habría que preocuparse del anclaje de esta  a la base, ni del desacoplo respecto a los imanes.
Esta idea se basa en un sintonizador, que mediante su desplazamiento a lo largo de la lámina va acortando o alargando la longitud vibrante de la misma.
//dibujo del sintonizador.
...
La misión de este, es fijar la lámina en distintas posiciones consiguiendo el efecto de variación de longitud de lamina que vibra sin modificar la longitud total, permitiendo una sintonización simple y efectiva.
Esta solucion ofrece una manera sencilla, efectiva y eficiente de sintonizar el sistema.
(Ver  pag 5 strategies for increasing the operating frecuency range of vibration energy harvesters.pdf)
...
%-------------------------------------------------------------------
De todos estos métodos presentados se va a implementar como primera aproximación la sintonización mecánica mediante la variación de la longitud efectiva a cargo de un sintonizador.

//dibujo del sistema con sintonizador. Sin tornillo
...
Estos motores proporcionan un movimiento rotatorio por lo que el siguiente paso es encontrar una forma de transformar este movimiento en un movimiento lineal.
 Tras barajar varias opciones,  (correa pasadora,  engranages) la manera que se ha pensado que es mas efectiva es mediante  un tornillo sin fin conectado a la vez al motor y al sintonizador.
De esta manera, mediante el giro del motor, gira el tornillo, y este a su vez, al pasar por una tuerca incrustada en el sintonizador hace que este se desplace transformando el movimiento circular en movimiento lineal. 
Volviendo a la [la tabla de maneras de sintonizar] se aprecia que los métodos de sintonización mecánica, en concreto cambiar las dimensiones, como es el método que aquí se utiliza, no es aplicable ?in situ?, es decir si el generador esta en funcionamiento, sin embargo mediante la técnica del sintonizador, ese problema desaparece y el generador puede ser sintonizado aun cuando este activo. Esta es una mejora conseguida con este proyecto.
...
Dentro de esta familia hay varios controladores. Desde PIC10 hasta PIC16.
...
Por otro lado hay dos puntos de donde se puede extraer y medir esta señal. El primero es a la salida de la bobina y el segundo es a la salida del multiplicador de Villard.
//representaciones de las tensiones a la salida de la bobina y del multiplicador
Salida de la bobina: Como se ve en la figura (figura donde se ponga lo q genera la bobina) en este punto la señal , sinusoidal centrada en 0, presenta dificultades para su captura por dos razones:
...
Salida del multiplicador: En este punto hay un nivel de tensión continua comparable a los valores de funcionamiento del microcontrolador y por lo tanto es más adecuado para tomar las medidas.
...
//insertar imagen algoritmo_ejemplo.jpg
...
En la imagen [algoritmo_ejemplo.jpg] estos dos conceptos aparecen juntos en un intento de dar una imagen general del sistema.
...
// insertar imagen algoritmo_simple.jpg
...
//insertar imagen algoritmo_lineal_mal.jpg
